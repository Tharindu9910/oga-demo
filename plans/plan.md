...
Phases 3–4 are **done** — a real Sanity project exists (`vl1egzty`/`production`) with real schemas, Studio structure, GROQ queries, and every page migrated off the mock content layer (`src/content/` has been deleted). See phases 3/4 below for what actually shipped, which differs in a few places from this section's original pre-Sanity-project draft (notably: no `/api/revalidate` webhook — the Live Content API handles freshness instead; draft-mode/Presentation Tool/Visual Editing deferred to a future phase).
...
**3. Sanity schemas + Studio — done.** `src/sanity/schemaTypes/` (documents: `project`, `event`, `chapter`, `post`; singletons: `homePage`, `aboutPage`, `projectsPage`, `membershipPage`, `loyaltyPage`, trimmed to what `Docs/editable_content.md` actually asks for — About and Loyalty each dropped several mock-layer sections that have no corresponding CMS requirement; those sections are now hardcoded directly in their page components). `structure.ts` pins the 5 singletons and groups Projects (Ongoing/Completed) and Events (Upcoming/Past) into filtered `S.documentList()` views. FAQ has no schema — it's hardcoded on the FAQ page, same pattern as Active Volunteer/Sports.

**4. Data layer — done.** `src/sanity/queries.ts` (GROQ via `defineQuery`, the only file with inline GROQ) + `src/sanity/lib/content.ts` (fetch wrappers, same function names/signatures the mock layer used) replace `src/content/` entirely, which has been deleted. Uses `next-sanity`'s Live Content API (`sanityFetch`/`SanityLive` from `src/sanity/lib/live.ts`, rendered in the root layout) rather than a hand-rolled tag/webhook system — publishing in the Studio updates the live site within seconds with no `/api/revalidate` route needed. `SanityImage` (`src/components/ui/SanityImage.tsx`) wraps `next/image` with a Sanity-CDN-transform `src` and `unoptimized` to skip Vercel's own image optimizer; `PortableTextBody` (`src/components/ui/PortableTextBody.tsx`) renders portable-text fields via `@portabletext/react`. `scripts/seed.ts` (`pnpm seed`) migrated the original mock JSON content into the real dataset one time. **Known GROQ gotcha:** `dateTime(date) >= now()` silently returns `null`/no matches in this API version — always cast both sides, `dateTime(date) >= dateTime(now())`. Still not done: draft-mode routes, the Presentation tool, and `<VisualEditing/>` — deferred to a later phase since this one was already large.
...
**8. Content seed:** compress the video with ffmpeg. The one-time `scripts/seed.ts` mentioned here was built and run in Phase 4 (migrating the mock JSON content into Sanity) rather than waiting for this phase — remaining work here is just the video compression.
